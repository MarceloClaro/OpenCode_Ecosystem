...
\begin{fichamento}
    \textbf{Ref.:} \texttt{docs/cotejo-citacoes-mh.md}

    \textbf{Conceitos-chave:} cotejo, citações, \MH{}, verificação, referências, documentação formal.

    \textbf{Descrição:} Documento formal de cotejo sistemático de todas as citações extraídas da \MH{} no manuscrito. Registra cada passagem citada com sua referência precisa (seção, parágrafo), verifica a fidelidade das transcrições, identifica paráfrases e articulações indiretas, e atesta a correção e adequação do uso da encíclica como fundamento normativo da pesquisa.

    \textbf{Articulação:} O cotejo constitui o instrumento de controle de qualidade acadêmica da pesquisa, garantindo a rastreabilidade e a fidelidade das referências à \MH{} em todos os capítulos do manuscrito.
\end{fichamento}

\begin{fichamento}
    \textbf{Ref.:} \texttt{STATUS.md}

    \textbf{Conceitos-chave:} status, progresso, relatório, documentação, cotejo, validação.

    \textbf{Descrição:} Arquivo de status atualizado do projeto de pesquisa. Registra o andamento de cada etapa, os marcos concluídos (incluindo o cotejo de citações da \MH{}), as pendências identificadas e as próximas ações previstas. Serve como instrumento de gerenciamento e documentação do ciclo de produção acadêmica.

    \textbf{Articulação:} O STATUS.md complementa o cotejo formal ao documentar o estado geral do projeto, permitindo a rastreabilidade do processo de pesquisa e validação.
\end{fichamento}
