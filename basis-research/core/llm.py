...
class LLMClient:
    ...
    def _check_ollama_active(self) -> bool:
        try:
    ...
    def _check_lmstudio_active(self) -> bool:
        try:
            resp = requests.get(f"{LM_STUDIO_BASE_URL}/models", timeout=2)
            return resp.status_code == 200
        except Exception:
            return False
    ...
    def _call_lmstudio(
        ...
        model = self._get_lmstudio_model()
        ...
        try:
            logger.info(f"[{agent_name}] LM Studio call — model: {model}")
            resp = requests.post(url, json=payload, headers=headers, timeout=300)
            resp.raise_for_status()
            data = resp.json()
            text = data["choices"][0]["message"]["content"]
            logger.info(f"[{agent_name}] LM Studio success — {len(text)} chars returned")
            return text
        except Exception as e:
            logger.error(f"[{agent_name}] LM Studio error: {e}")
            return None
    ...
    def call(
        ...
        Priority:
          ...
          3. Local Provider (LM Studio if active, otherwise Ollama)
          4. Ollama light model (if Ollama used and primary fails)
        ...
        # Step 3: Local LLM Fallback (Ollama or LM Studio)
        provider = os.environ.get("LOCAL_LLM_PROVIDER", "auto").lower()
        if provider == "auto":
            if self._check_lmstudio_active():
                provider = "lmstudio"
            else:
                provider = "ollama"

        if provider == "lmstudio":
            logger.info(f"[{agent_name}] Falling back to LM Studio")
            result = self._call_lmstudio(prompt, system, agent_name)
            if result:
                return result

        # Step 4: Try Ollama primary model
        ...
        raise RuntimeError(
            ...
            f"Check ANTHROPIC_API_KEY, LM Studio, or Ollama availability."
        )
