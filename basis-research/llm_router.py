...
class LLMClient:
    ...
    def call(
        ...
        if provider == "lmstudio":
            logger.info(f"[{agent_name}] Fallback → LM Studio")
            self._publish("llm.fallback", {"agent": agent_name, "to": "lmstudio"})
    ...
    def _call_lmstudio(self, prompt: str, system: str, agent_name: str) -> Optional[str]:
        ...
        try:
            ...
            self._publish("llm.success", {"agent": agent_name, "model": f"lmstudio:{model}",
                                           "chars": len(text)})
            return text
        except Exception as e:
