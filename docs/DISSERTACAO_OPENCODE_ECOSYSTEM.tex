...
\begin{enumerate}
    ...
    \item \textbf{Chat Local e RAG Híbrido (\texttt{chat_ollama.py})}: Orquestrador local integrado à engine de inferência \texttt{Ollama} com o modelo \texttt{qwen2.5-coder:1.5b} otimizado para CPUs. Possui interface de diálogo (White-Box Streaming) com auditoria detalhada de consumo de tokens e logs de custos economizados (\texttt{docs/session_token_audit.log}). O pipeline executa RAG híbrido consultando códigos locais no diretório \texttt{projects/} e a API externa da Wikipedia e DuckDuckGo.
    
    \item \textbf{Daemon de Vocalização de Áudio (\texttt{vocalizer_daemon.ps1})}: Serviço em background em Windows PowerShell que monitora o arquivo de comunicação assíncrona \texttt{.vocalizer_cmd}. Invoca classes nativas do framework .NET (\texttt{System.Speech.Synthesis.SpeechSynthesizer}) para ler textos higienizados de relatórios em português brasileiro, suportando ações de pausar (\texttt{PAUSE}), retomar (\texttt{RESUME}), repetir (\texttt{REPEAT}) e parar (\texttt{STOP}) via atalhos do teclado.
\end{enumerate}

O alinhamento e integridade desses módulos de interface são validados no pipeline de conformidade TDD pelo caso de teste \textbf{T6} do validador de ambiente \texttt{test_environment.sh}, garantindo que a infraestrutura completa esteja operacional antes da interação com o usuário.
