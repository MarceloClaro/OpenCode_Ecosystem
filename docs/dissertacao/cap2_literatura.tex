...
Metacognição — literalmente ``cognição sobre a cognição'' — é a capacidade de refletir sobre os próprios processos mentais. O termo foi cunhado por Flavell (1979) \doi{10.1037/0003-066X.34.10.906} no contexto da psicologia do desenvolvimento, mas suas implicações se estendem muito além.

Para o leitor não familiarizado com o conceito, uma analogia cotidiana: quando você está lendo um texto e subitamente percebe que não entendeu as últimas três frases, você acaba de exercer metacognição. Você não apenas leu (cognição) — você \textit{monitorou} sua própria compreensão da leitura (metacognição) e, ao perceber a falha, provavelmente voltou para reler o trecho (regulação metacognitiva).

Flavell (1979) distinguiu dois componentes fundamentais da metacognição que permanecem centrais até hoje. O primeiro é o \textbf{conhecimento metacognitivo}: o que sabemos sobre nosso próprio funcionamento cognitivo. Este conhecimento se divide em três categorias: conhecimento sobre pessoas (``eu sou melhor em matemática do que em poesia''), conhecimento sobre tarefas (``este problema é difícil porque envolve muitas variáveis''), e conhecimento sobre estratégias (``quando enfrento um problema complexo, faço um diagrama primeiro''). O segundo componente é a \textbf{regulação metacognitiva}: como monitoramos e controlamos ativamente nossos processos cognitivos. Isto inclui planejamento (``por onde devo começar?''), monitoramento (``estou progredindo adequadamente?''), e avaliação (``minha solução está correta? Como posso verificá-la?'').
...
O modelo de memória de Atkinson e Shiffrin (1968) \doi{10.1016/S0079-7421(08)60422-3} é um dos modelos mais influentes e duradouros da psicologia cognitiva. Publicado há mais de meio século, continua sendo ensinado em cursos introdutórios de psicologia e informa o design de sistemas de memória artificial até hoje.
...
John Nash (1950) \doi{10.1073/pnas.36.1.48} demonstrou que todo jogo com um número finito de jogadores e estratégias tem pelo menos um ponto de equilíbrio — uma configuração onde nenhum jogador tem incentivo para mudar unilateralmente sua estratégia. O equilíbrio de Nash é aplicado no \texttt{DialecticalEngine} (Seção 6.2) como modelo de convergência: tese e antítese são tratadas como estratégias concorrentes, e a síntese é o ponto de equilíbrio onde ambas encontram expressão máxima sem contradição.
