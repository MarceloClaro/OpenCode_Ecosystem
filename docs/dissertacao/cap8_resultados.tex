...
Para que um auditor independente possa verificar todos os resultados apresentados nesta dissertação, o seguinte comando reproduz a evidência completa:
...
\section{Estudos de Caso — Evidência Experimental}
...
\begin{enumerate}
    \item \textbf{R17 (CT-007 — Evolve Pipeline)}: O CT-007 do SPEC-026 falhou por 3 ciclos consecutivos devido a uma única linha corrompida (\texttt{l\}}) no arquivo \texttt{ecosystem-observability.jsonl} de 8.001 linhas. A falha era intermitente — aparecia apenas quando o parser JSON atingia a linha corrompida. Este episódio revelou que \textbf{observabilidade sem validação de integridade é ilusória}. Em resposta, o \texttt{AnomalyDetector} (SPEC-036) foi estendido com verificação de integridade dos logs JSONL antes de cada análise. Lição: \textit{todo sistema de monitoramento deve primeiro monitorar a si mesmo}.
    
    \item \textbf{R20 (Integração SPEC-033)}: A primeira tentativa de integrar o CapabilityComposer ao pipeline quebrou 6 CTs do SPEC-030. Diagnóstico: o \texttt{capability\_id} usava \texttt{gap.category} (ex.: ``Quantitativo experimental''), mas os cenários esperavam \texttt{domain.category} (ex.: ``metodos.Quantitativo experimental''). A correção foi uma linha (\texttt{f"{gap.dim\_key}.{gap.category}"}), mas o diagnóstico levou 4 horas. Lição: \textit{a integração entre camadas exige validação de contrato de dados} — princípio formalizado como requisito para todas as SPECs subsequentes.
\end{enumerate}
...
\textbf{Contexto}: O TrustEngine (SPEC-038) foi testado em cenário simulado de produção com 3 categorias de ações: confiável (10 execuções bem-sucedidas consecutivas), arriscada (3 sucessos seguidos de 5 falhas), e nova (sem histórico, shadow mode).
