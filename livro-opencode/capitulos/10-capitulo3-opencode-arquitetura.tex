...
\begin{exercicio}
Identifique no código-fonte os arquivos \codigo{core/container.py},
\codigo{core/orchestrator.py} e \codigo{core/agent\_manager.py}.
Descreva as responsabilidades de cada um. (\nivelbasico)
\end{exercicio}
...
\item \textbf{2 plugins .ts locais}: plug-ins TypeScript no diretório
\codigo{plugins/} (ex.: \codigo{manus-evolve.ts});
...
\begin{tikzpicture}
  ...
  \foreach \name/\count/\color/\y in {
    ...
    \pgfmathsetmacro{\width}{\count * 0.08}
    \filldraw[fill=\color!20, draw=\color!50!black]
      (0, \y) rectangle (\width, \y+0.4);
    \node[font=\small] at (\width+0.3, \y+0.2) {\name{} (\count)};
  }
  \draw[->] (0,-1.7) -- (10.5,-1.7);
  \node[font=\small] at (10.5, -2.0) {Quantidade de skills};
\end{tikzpicture}
\end{figure}
...
\codigo{analisador-estrutural}. Documente os padrões de projeto
identificados. (\nivelavancado)
\end{exercicio}
...
\begin{enumerate}
\item Clone o repositório: \codigo{git clone
https://github.com/marceloclaro/opencode-ecosystem.git};
\item Instale dependências Python: \codigo{pip install -r
requirements.txt};
...
\url{https://github.com/marceloclaro/opencode-ecosystem/tree/main/specs}.}
\footnotetext[3]{ADRs do ecossistema:
\url{https://github.com/marceloclaro/opencode-ecosystem/tree/main/adr}.}
...
\url{https://github.com/marceloclaro/opencode-ecosystem/tree/main/diagrams}.}
...
\url{https://github.com/marceloclaro/opencode-ecosystem/tree/main/skills}.}
