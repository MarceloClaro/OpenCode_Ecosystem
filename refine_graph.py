...
def infer_edges_between_nodes(conn):
    ...
    # Skill -> MCP: check path for MCP name references
    for s in skills:
        desc = (s["path"] or "")
        for m in conn.execute("SELECT id, name FROM graph_nodes WHERE type='mcp'").fetchall():
            mname = m["name"].lower()
...
def add_lgpd_tags(conn):
    keywords = {
        "lgpd": ["lgpd", "privacidade", "privac", "dados pessoais", "data protection",
                 "gdpr", "anonimiz", "pseudonimiz"],
        "ethical_ai": ["etica", "etic", "integridade", "transparencia", "transparenc",
                       "ia", "inteligencia artificial", "ai", "artificial intelligence"],
        "research_compliance": ["pesquisa", "academico", "cientifico", "universidade",
                                "ppg", "ufc", "regulacao", "conformidade", "compliance"],
    }
...
def main():
    ...
    print(f"  {nn} entities, {ne} entity edges")
