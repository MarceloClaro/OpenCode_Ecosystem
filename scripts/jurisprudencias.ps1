...
function Search-JurDecisions {
    <#
    .SYNOPSIS
      Busca decisoes por tribunal com cache. Consome 1 busca/dia se nao estiver em cache.
    .PARAMETER Court
      Slug do tribunal (stf, stj, tjsp, tjrs, trf3, etc).
    .PARAMETER Query
      Termo de busca FTS5 (AND, OR, NOT, frase exata entre aspas, prefixo*).
    .PARAMETER Page
      Pagina (0-based, 10 resultados). Padrao: 0.
    .PARAMETER PubFrom
      Data publicacao inicial (ISO, ex: 2025-01-01).
    .PARAMETER PubTo
      Data publicacao final.
    .PARAMETER TrialFrom
      Data julgamento inicial.
    .PARAMETER TrialTo
      Data julgamento final.
    .PARAMETER Force
      Ignora cache e força requisicao a API.
    .PARAMETER NoCache
      Nao le nem escreve cache (requisicao direta).
    #>
    param(
...
function Get-JurDecision {
    <#
    .SYNOPSIS
      Consulta decisao por numero do processo. NAO consome cota de busca.
    .PARAMETER Court
      Slug do tribunal.
    .PARAMETER Number
      Numero do processo (ex: ARE 1587011 AgR).
    #>
    param(
        [Parameter(Mandatory)] [string] $Court,
...
function Clear-JurCache {
    ...
    .PARAMETER OlderThanHours
      Remove apenas entradas mais antigas que N horas.
    #>
...
function Search-JurCache {
    <#
    .SYNOPSIS
      Busca textual offline em todo o cache local. NAO consome cota diaria.
    .PARAMETER Term
      Termo para buscar nos campos excerpt, process_number e (se cache rico) summary.
    .PARAMETER Court
      Filtro opcional por tribunal.
    .PARAMETER SimpleOutput
      Exibe apenas processo + data (formato compacto).
    #>
    param(
...
function Export-JurDocs {
    <#
    .SYNOPSIS
      Gera documentacao HTML standalone a partir do cache local para consulta offline.
    .PARAMETER OutputPath
      Caminho do arquivo .html de saida. Padrao: .\jurisprudencias_offline.html
    .PARAMETER Court
      Filtro opcional por tribunal.
    .PARAMETER Open
      Abre o HTML no navegador apos gerar.
    #>
    param(
    ...
    _ensure-cache-dir
    $files = Get-ChildItem "$($Script:CacheDir)\*.json" -ErrorAction SilentlyContinue
    if (-not $files) { Write-Host "Cache vazio. Nada para exportar."; return }
    ...
    foreach ($f in $files) {
        try {
            ...
            $cacheKey = ($f.BaseName -split '\|')[1]  # extrai termo aproximado
            if ($cacheKey) { $termsIndex[$cacheKey] = $true }
    ...
    $courts = ($allDecisions | ForEach-Object { $_.court } | Where-Object { $_ } | Select-Object -Unique) -join ", "

    $rows = $allDecisions | Sort-Object publication_date -Descending | ForEach-Object {
        $num = $_.process_number
        $date = $_.publication_date
        $trial = $_.trial_date
        $court = $_.court
        $rel = $_.rapporteur
        $body = $_.excerpt
        ...
        $urlHtml = if ($url) {
            $u = [System.Net.WebUtility]::HtmlEncode($url)
            "<a href=`"$u`" target=`"_blank`">$u</a>"
        } else { "" }

        @"
        <div class="card">
            <div class="card-title"><a href="#$([System.Net.WebUtility]::HtmlEncode($num))" name="$([System.Net.WebUtility]::HtmlEncode($num))">$([System.Net.WebUtility]::HtmlEncode($title))</a></div>
            <div class="card-meta">$metaStr</div>
            $bodyHtml
            <div class="card-url">$urlHtml</div>
        </div>
"@
    }
...
<title>Jurisprudencias - Documentacao Offline</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; background: #f5f5f5; color: #222; line-height: 1.6; padding: 2rem; }
  ...
  .card-title { font-weight: 600; font-size: 1.05rem; margin-bottom: 0.35rem; }
...
<div class="container">
  ...
  <p class="subtitle">Gerado em $(Get-Date -Format "dd/MM/yyyy HH:mm") &middot; Dados do cache local</p>
  <div class="stats">
    ...
    <div class="stat"><div class="stat-num">$($($allDecisions | ForEach-Object { $_.process_number } | Where-Object { $_ } | Select-Object -Unique).Count)</div><div class="stat-label">Processos</div></div>
  </div>
  <div class="search-box">
    <input type="text" id="filter" placeholder="Filtrar por palavra-chave..." oninput="filterCards()">
  </div>
  <div id="cards">
    $rows
  </div>
  <div class="footer">
    Dados provenientes de api.jurisprudencias.ai &middot; Cache TTL: $($Script:CacheTTLHours)h
  </div>
</div>
<script>
...
function Invoke-JurPreFetch {
    <#
    .SYNOPSIS
      Pre-busca decisoes para varios termos de uma vez (consome cota diaria).
      Use no inicio do dia antes de atingir o rate limit.
    .PARAMETER Court
      Tribunal alvo.
    .PARAMETER Terms
      Lista de termos para buscar. Padrao: termos uteis para direito publico.
    .PARAMETER MaxPages
      Paginas por termo (0-based, 10 decisoes cada). Padrao: 0 (primeira pagina).
    #>
    param(
        [Parameter(Mandatory)] [string] $Court,
        [string[]] $Terms = @(
            "desapropriacao", "precatorio", "imissao na posse",
            "indenizacao", "utilidade publica", "justa indenizacao",
            "tombamento", "direito de propriedade", "posse",
            "reintegracao de posse", "usucapiao", "bem publico",
            "servidao administrativa", "limitação administrativa"
        ),
        [int] $MaxPages = 0
    )
    Write-Host "=== PRE-FETCH: $Court ===" -ForegroundColor Yellow
    Write-Host "Termos: $($Terms.Count) | Paginas por termo: $($MaxPages + 1)" -ForegroundColor DarkGray

    $total = 0
    foreach ($term in $Terms) {
        for ($p = 0; $p -le $MaxPages; $p++) {
            Write-Host "[$($Court)] buscando: '$term' pag.$p ... " -NoNewline
            try {
                $r = Search-JurDecisions -Court $Court -Query $term -Page $p -Force
                if ($r -and @($r).Count -gt 0) {
                    Write-Host "$(@($r).Count) resultados" -ForegroundColor Green
                    $total += @($r).Count
                } else {
                    Write-Host "0 resultados" -ForegroundColor DarkGray
                    break
                }
            } catch {
