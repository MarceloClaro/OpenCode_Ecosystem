...
4.  **`NCBI_API_KEY`** (optional): Raises the NCBI rate limit from 3 to 10
    ...
    the user this command â€” **substituting `ENV_FILE` with the resolved literal
...
-   **Use the Wrapper**: Prefer the wrapper script for standard queries. It
    ...
    may modify the script or query the NCBI E-utilities API directly â€” but be
...
### 1. `count` â€” Count Matching Variants
...
-   `total_count` â€” Total number of matching variants in ClinVar.
-   `fetched_count` â€” Number of IDs actually retrieved.
-   `variant_ids` â€” List of ClinVar Variation ID strings.

### 3. `summary` â€” Get Interpretation Summary
...
-   `genes` â€” list of `{gene_id, symbol, strand}`
-   `variation_type` â€” e.g., single nucleotide variant, Deletion, Insertion
-   `molecular_consequences` â€” list of strings (e.g., ["missense variant", \
    "nonsense"])

### 4. `evidence` â€” Get Clinical Evidence
...
-   `variant_id`
-   `allele_info` â€” `{chromosome, position_start, position_stop,
    reference_allele, alternate_allele, cytogenetic_band, dbsnp_rsid}` (GRCh38
    preferred)
-   `conditions` â€” list of `{name, medgen_cui, omim_id, orphanet_id, hpo_terms}`
-   `functional_consequences` â€” list of `{value, sequence_ontology_id}`
-   `structural_variant_details` â€” `{outer_start, inner_start, inner_stop,
    outer_stop, copy_number}` (present only for CNVs, otherwise null)
-   `citation_references` â€” list of PubMed IDs cited in the global "Citations"
    section
-   `submissions` â€” list of per-submitter records, each containing:
    -   `submitter_name`, `classification`, `curator_notes`,
        `assertion_criteria`
    -   `date_last_evaluated` â€” when the submitter last reviewed the
        classification
...
```bash
# Step 1: Gauge size (optional â€” search also returns total_count)
...
### Deep Dive: search â†’ evidence

When you need the full clinical picture for a specific variant â€” including
submitter rationales, PubMed citations, ontology-linked conditions, and allele
coordinates â€” use `evidence`.
...
-   `c.-â€¦` â€” 5' UTR (non-coding)
-   `c.*â€¦` â€” 3' UTR (non-coding)
-   `c.123+N` / `c.123-N` â€” intronic (non-coding)
-   `p.Trp146Arg` etc. â€” protein effect (coding)
...
-   **Quick pathogenicity label** â€” Use `summary` â†’ `clinical_significance`
-   **Gene symbol and strand** â€” Use `summary` â†’ `genes`
-   **Variant type (SNV, del, etc.)** â€” Use `summary` â†’ `variation_type`
-   **Protein-level effect** â€” Use `summary` â†’ `molecular_consequences`
-   **Genomic coordinates (GRCh38)** â€” Use `evidence` â†’ `allele_info`
-   **Linked conditions (ontology)** â€” Use `evidence` â†’ `conditions`
-   **SO functional consequence** â€” Use `evidence` â†’ `functional_consequences`
-   **CNV breakpoints/copy number** â€” Use `evidence` â†’
    `structural_variant_details`
-   **PubMed references** â€” Use `evidence` â†’ `citation_references`
-   **Date of last lab review** â€” Use both â†’ `last_evaluated`
-   **Clinician rationales** â€” Use `evidence` â†’ `submissions[].curator_notes`
...
-   The `search` command fetches all results by default and includes
    `total_count` and `fetched_count` in the output â€” always verify these match
...
-   **Case sensitivity in molecular consequences** â€” ClinVar returns mixed-case
    strings. Always use case-insensitive matching (`.lower()`) when filtering.
-   **Parsing `search` output as a bare list** â€” `search` returns a JSON object
    with `total_count`, `fetched_count`, and `variant_ids` â€” not a bare list.
