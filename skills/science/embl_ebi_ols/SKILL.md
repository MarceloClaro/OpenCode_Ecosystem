...
## When to Use â€” Quick Recipes

Use this skill whenever a user query matches one of these patterns:

-   **Definition** of a disease, phenotype, or term â†’ `get_term.py --obo_id <ID>
    --summary`
-   **Subtypes** or **children** of a term â†’ `get_term.py --obo_id <ID>
    --relations children`
-   **Parent** of a term â†’ `get_term.py --obo_id <ID> --relations parents`
-   **Ancestors** / disease **categories** / **classified under** â†’ `get_term.py
    --obo_id <ID> --relations ancestors`
-   **Root terms** of an ontology â†’ `get_term.py --ontology <id> --roots`
-   **Hierarchical** parents (is-a + part-of) â†’ `get_term.py --obo_id <ID>
    --relations hierarchicalParents`
-   **Structures part of** / hierarchical children â†’ `get_term.py --obo_id <ID>
    --relations hierarchicalChildren`
-   **Compare** direct vs hierarchical parents â†’ `get_term.py --obo_id <ID>
    --relations parents,hierarchicalParents`
-   Search for a term (e.g., "apoptosis" in GO) â†’ `search_ols.py --query "..."
    --ontology <id>`
-   Find a **GO term** matching a function â†’ `search_ols.py --query "..."
    --ontology go --exact`
-   Search in **MONDO**, **CHEBI**, **CL**, **UBERON** â†’ `search_ols.py --query
    "..." --ontology <id> --defining`
-   **Paginate** search results / next page â†’ `search_ols.py --query "..."
    --rows N --start <offset>`
-   Autocomplete a partial name â†’ `suggest_ols.py --query "..."`
-   Ontology metadata (e.g., EFO info) â†’ `get_ontology.py --id <id>`
-   OLS index statistics â†’ `get_stats.py`
...
> **exactly 2 steps** â€” do NOT search across multiple ontologies:
>
...
-   `--ontology`: Filter by ontology ID (e.g., `go`, `doid`, `efo`, `hp`).
    **Recommended** when you know which ontology to search â€” avoids noise from
    250+ ontologies.
...
-   `--relations`: Comma-separated list of relations to fetch.
    ...
    -   **Graph:** `graph` â€” full graph JSON for a term
