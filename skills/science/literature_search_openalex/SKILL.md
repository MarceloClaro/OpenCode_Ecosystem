...
4.  **`OPENALEX_API_KEY`** (optional but recommended): Enables the OpenAlex
    ...
    context). Instead, give the user this command â€” **substituting `ENV_FILE`
...
5.  **API key.** If a command returns 401/429 or you need high-volume queries,
    ...
    `OPENALEX_API_KEY` to the `.env` file. Keys are at OpenAlex.org â†’ account
    settings.
6.  **Keep output small.** Always use `--select` and `--per-page 5â€“10` for
...
**resolve** `<entity> <query>` â€” Name â†’ ID candidates. Returns `id`,
`display_name`, `hint`. Use `--per-page N` for more candidates.

**get** `<entity> <id>` â€” Full metadata for one entity. Accepts short ID
(`W2741809807`), full URL, or DOI URL. Use `--select` to limit fields.

**filter** `<entity>` â€” Search/filter entities. Key flags are:

-   `--search <query>`: Full-text search (10Ã— cost of `--filter`)
...
**download-pdf** `<work-id> <output-path>` â€” Download PDF (requires API key).
...
**rate-limit** â€” Check current rate limit status (requires API key).
...
-   [Works](references/works.md) â€” [Authors](references/authors.md) â€”
    [Sources](references/sources.md)
-   [Institutions](references/institutions.md) â€” [Topics](references/topics.md)
    â€” [Taxonomy](references/taxonomy.md)
-   [Geo & Language](references/geo_and_language.md) â€”
...
```bash
# Author's works (resolve â†’ filter)
...
Never fabricate results on empty responses â€” report accurately and suggest
