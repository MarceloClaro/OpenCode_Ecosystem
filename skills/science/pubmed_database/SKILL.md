...
these commands â€” **substituting `ENV_FILE` with the resolved literal path to the
`.env` file**:
...
`scripts/pubmed_api.py` â€” a single CLI with 10 functions covering search, fetch,
...
5.  **Search Termination**: When asked to find papers that may not exist, limit
    exploration to 3â€“5 high-quality, varied search queries. If no results match
    after these attempts, conclude that no papers meet the criteria rather than
    continuing to iterate â€” unless explicitly instructed to be thorough.

## Functions

> **âš ï¸ MANDATORY**: You **MUST** read the linked reference file for a function
> group **before calling any function** in that group. The tables below only
> describe *what* each function does â€” not *how* to call it. Argument names,
...
for complete workflow recipes (searchâ†’fetch, cross-db exploration, citation
