...
All PyMOL commands run on the host via `uv` with OSMesa software rendering â€” no
...
-   The user only has a sequence and no structure file â€” fetch the structure
...
-   **Software rendering only.** Use `cmd.png()` for output. Never use
    `cmd.draw()` or `cmd.ray()` with hardware acceleration â€” OSMesa does not
...
-   **Cartoon with secondary structure coloring** â€” basic helix/sheet/loop
    coloring
-   **B-factor (pLDDT) coloring** â€” continuous spectrum coloring by B-factor
-   **AlphaFold pLDDT coloring** â€” canonical threshold-based confidence colors
-   **Highlight specific residues** â€” show active site or key residues as sticks
-   **Surface rendering** â€” transparent surface over cartoon
-   **Electrostatic surface rendering** â€” vacuum electrostatics (qualitative)
-   **Multi-chain complex colors** â€” automatic per-chain coloring
-   **B-factor putty analysis** â€” tube width proportional to flexibility
-   **Cavity and pocket visualization** â€” surface cavity detection with ligand
    focus
-   **Multi-structure batch rendering** â€” render a directory of structures
-   **Measure distance between residues** â€” CAâ€“CA distance with labels
-   **Zoom into binding pocket** â€” simple pocket focus
-   **Protein-ligand interaction** â€” ligand isolation, styled rendering, polar
    contacts
-   **Two-structure superposition with RMSD** â€” align/cealign with auto-fallback
-   **In silico mutagenesis** â€” mutate residues with the mutagenesis wizard
-   **Load and modify an existing session** â€” re-open a `.pse` file
