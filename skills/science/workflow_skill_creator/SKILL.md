...
new skill **MUST** reference it â€” do not offer a self-contained option.
...
2.  Determine whether the skill needs any code. If any step involves calling an
    ...
    **needs code** and you should default to the CLI pattern. Only use a
    ...
    -   If code is needed: "Some of these steps involve [fetching data from an
        ...
        commands like `search`, `fetch`, `analyze`, etc. â€” you won't need to
        write any code yourself. Does that sound right?"
    -   If no code is needed: "This workflow is entirely about following a set
        of steps and using existing tools â€” no new code is needed. I'll write it
...
-   [ ] Does the workflow need any code? (If yes â†’ CLI pattern; if no â†’
    instruction-only)
...
1.  **Skill name and description** (following YAML frontmatter rules: name â‰¤64
    chars, lowercase + hyphens; description â‰¤1024 chars).
...
-   Rate limits must be documented and respected in code. Prefer
    **file-lockâ€“based rate limiting** so that concurrent sub-agents sharing the
    same machine collectively respect the limit. See other skills in the Science
    Skills bundle for the canonical cross-processâ€“safe implementation.
...
template at `references/cli_script_template.py` â€” see the `RateLimitError` class
...
-   On non-retriable HTTP errors (e.g. 400, 403, 404), read and include the
    response body in the error message â€” not just the status code. API response
...
-   File output is token-efficient â€” the agent reads only the fields it needs
...
â€” i.e., every step is purely about orchestration, reasoning, multi-skill
